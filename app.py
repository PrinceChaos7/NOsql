...
def agregar_paciente_endpoint():
    try:
        ...
        if not nuevo_paciente:
            return jsonify({"mensaje": "El paciente ya existe"}), 409
        return jsonify(json_serializable(nuevo_paciente)), 201
...
def agregar_registro_endpoint(ci):
    try:
        ...
        return jsonify(json_serializable(nuevo_registro)), 201
    except Exception as e:
        return jsonify({"mensaje": f"Error interno del servidor: {str(e)}"}), 500
